class Api::ServiceRequestsController < ApplicationController
 ...
 def user_last_completed
                 ...
                 @userid=params[:user_id]
         @request = ServiceRequest.where("user_id=? and (status=? or status=? ) " , @userid,"Completed" ,"Completed").order('service_requests.date DESC').order('service_requests.request_time DESC').limit(1) 
